...
export function mapApiStock(raw: Record<string, unknown>): Stock {
  return {
    ...
    cashflow5Q:     (raw.cashflow_5q as boolean) ?? false,
    pe:             (raw.pe as number | null)    ?? null,
    ...
    priceTarget:    (raw.price_target as number) ?? 0,
    ...
    high52w:        (raw.high52w as number)      ?? undefined,
    low52w:         (raw.low52w as number)       ?? undefined,
    // ── Tier 2 ──────────────────────────────────────────────
    ...
    rsVsNifty:      (raw.rs_vs_nifty as boolean)     ?? false,
