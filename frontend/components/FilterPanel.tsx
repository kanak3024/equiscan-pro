'use client'
import { Filters, TIER2_FILTERS } from '../lib/screenerEngine'
...
type FilterDef = {
  ...
  label: string
  hint: string
  cat: 'LISTING' | 'QUALITY' | 'TECH' | 'FUND' | 'ANALYST' | 'TIER2'
}
...
const filterDefs: FilterDef[] = [
  ...
  // Tier 2 — mapped in from TIER2_FILTERS constant
  ...TIER2_FILTERS.map(f => ({
    key:  f.key,
    label: f.label,
    hint:  f.description,
...
const catLabels: Record<string, string> = {
  ...
  FUND:    'FUNDAMENTAL',
  ANALYST: 'ANALYST',
  TIER2:   'TIER 2 — PRO SIGNALS',
}

const categories = ['LISTING', 'QUALITY', 'TECH', 'FUND', 'ANALYST', 'TIER2']

const RESET_STATE: Partial<Filters> = {
  f_listed8y: false, f_mcap1000: false, f_promoter75: false,
  f_noDilution: false, f_dmaAligned: false, f_dma50_200: false,
  f_profit3Q: false, f_cashflow5Q: false, f_analystPositive: false,
  f_pledge5: false, f_rsiHealthy: false, f_peUnder60: false,
  f_revenueGrowth15: false, f_debtLow: false, f_roe15: false,
  f_recentUpgrade: false, f_priceTargetUpside20: false, f_instBuying: false,
  f_epsIncreasing: false, f_near52wHigh: false,
  // tier 2
  f_macdCrossover: false, f_new3mHigh: false, f_goldenCross: false,
  f_rsVsNifty: false, f_lowAtr: false, f_bbSqueeze: false,
}
...
export default function FilterPanel({ filters, onChange }: Props) {
  ...
  const tier2Active = TIER2_FILTERS.filter(f => filters[f.key] === true).length
  ...
  return (
    <div className="bg-[#0f1218] border border-white/[0.07] rounded-xl p-5 sticky top-20 max-h-[calc(100vh-120px)] overflow-y-auto">
      ...
      {categories.map(cat => (
        <div key={cat} className="mb-5">
          ...
          <div className="text-[10px] font-mono uppercase tracking-widest mb-2 flex items-center gap-2">
            <span className={catColors[cat]}>{catLabels[cat]}</span>
            {cat === 'TIER2' && tier2Active > 0 && (
          ...
          {/* Tier 2 banner */}
          {cat === 'TIER2' && (
            <div className="mb-3 px-3 py-2 rounded-lg bg-rose-500/5 border border-rose-500/10 text-[10px] text-rose-300/60 font-mono leading-relaxed">
              Advanced signals used by retail & PMS desks.
              Powered by live Kite data — enable after market hours refresh.
            </div>
          )}
      ...
      {/* Reset */}
      <button
        onClick={() => onChange({ ...filters, ...RESET_STATE })}
